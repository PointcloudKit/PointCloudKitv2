...
final public class ProcessorService: ObservableObject {
    ...
    func voxelDownsampling(
        for object: Object3D,
        parameters: ProcessorParameters.VoxelDownSampling
    ...
    func statisticalOutlierRemoval(
        for object: Object3D,
        parameters: ProcessorParameters.OutlierRemoval.Statistical
    ) -> Future<Object3D, ProcessorServiceError> {
    ...
    func radiusOutlierRemoval(
        for object: Object3D,
        parameters: ProcessorParameters.OutlierRemoval.Radius
    ) -> Future<Object3D, ProcessorServiceError> {
    ...
    func normalsEstimation(
        for object: Object3D,
        parameters: ProcessorParameters.NormalsEstimation
    ) -> Future<Object3D, ProcessorServiceError> {
    ...
    func poissonSurfaceReconstruction(
        for object: Object3D,
        parameters: ProcessorParameters.SurfaceReconstruction.Poisson
    ) -> Future<Object3D, ProcessorServiceError> {
        // FAUT KI YE LE NORMAL
        process(object, with: [FaceProcessor.poissonSurfaceReconstruction(depth: parameters.depth)])
    }

    func convert(capture: PointCloudCapture) -> Future<Object3D, Never> {
        Future { promise in
            // Filter out the
...
extension ProcessorService {
    ...
    func convertObject3DPointCloud(_ object: Object3D) -> Open3DPointCloud {
        let points = object.vertices.map { vertex in numpy.ones([3], dtype: numpy.float64) }
        let colors = object.vertexColors.map { color in numpy.ones([3], dtype: numpy.float64) }
        let normals = object.vertexNormals.map { normal in numpy.ones([3], dtype: numpy.float64) }
        let pointCloud = o3d.PointCloud()
        pointCloud.points = o3d.Vector3dVector(points)
        pointCloud.colors = o3d.Vector3DVector(colors)
        pointCloud.normals = o3d.Vector3DVector(normals)
        return pointCloud
    }
    ...
    func convertObject3DTriangleMesh(_ object: Object3D) -> Open3DTriangleMeshes {
        ...
        let colors = object.vertexColors.map { color in numpy.ones([3], dtype: numpy.float64) }
        let normals = object.vertexNormals.map { normal in numpy.ones([3], dtype: numpy.float64) }
        let triangles = object.triangles.map { triangle in numpy.ones([3], dtype: numpy.int) }
        let triangleMeshes = o3d.TriangleMesh()
        triangleMeshes.vertices = o3d.Vector3dVector(points)
        triangleMeshes.vertex_colors = o3d.Vector3DVector(colors)
        triangleMeshes.vertex_normal = o3d.Vector3DVector(normals)
