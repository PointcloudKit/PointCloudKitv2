...
final public class CaptureViewerModel: ObservableObject {
    ...
    private let capture: PointCloudCapture
    ...
    func plyFile() -> PLYFile {
        PLYFile(particles: capture.buffer.getMemoryRepresentationCopy(for: vertexCount))
    }
    ...
    func undo() {
        ...
        DispatchQueue.global(qos: .userInteractive).async { [weak self] in
            ...
            self.capture.buffer.assign(with: previousPointCloud)
        ...
        DispatchQueue.global(qos: .userInteractive).async { [weak self] in
            ...
            self.pointCloudProcessor.process(self.plyFile(), with: processors)
                .sink(receiveCompletion: { result in
                    ...
                    self.capture.buffer.assign(with: downSampledParticles)
                    DispatchQueue.main.async {
